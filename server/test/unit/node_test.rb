...
class NodeTest < Test::Unit::TestCase
  ...
  def test_should_not_save_without_name
    node = Node.new
    node.hardware_profile = HardwareProfile.first
    node.status = Status.first
    assert !node.save, "Saved the Node without a name!"
  end
  def test_should_not_save_without_hardware_profile
    node = Node.new
    node.name = 'testnode1'
    node.status = Status.first
    assert !node.save, "Saved the Node without a hardware_profile!"
  end
  def test_should_not_save_without_status
    node = Node.new
    node.name = 'testnode1'
    node.hardware_profile = HardwareProfile.first
    assert !node.save, "Saved the Node without a status!"
  end
