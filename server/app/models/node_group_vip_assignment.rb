class NodeGroupVipAssignment < ActiveRecord::Base
  ...
  validates_presence_of :vip_id, :node_group_id
  validates_uniqueness_of :vip_id, :scope => :node_group_id
  ...
  def before_destroy
    ...
    if !virtual_assignment? 
      ...
      node_group.child_groups.each do |child|
        flag << child if  child.vips.include?(vip)
      end
  ...
  # Recursively ensure that each parent of this NGNA's vip group has a
  # NGNA to this NGNA's vip
  ...
  # Remove virtual assignments for this NGNA's vip from each of this NGNA's
  # vip group's parents recursively, unless the vip should have a virtual
  # assignment to that parent vip group due to other NGNAs.
  def remove_virtual_assignments_from_parents(deleted_ngna=self)
    node_group.parent_groups.each do |parent|
      # Check if the vip has reason to keep its assignment to this parent
      # vip group due to other NGNAs
      if parent.all_child_vips_except_ngna(deleted_ngna).include?(vip)
        logger.debug "Leaving NGNA from vip #{vip.name} to parent #{parent.name} due to other NGNAs"
      else
        ngna = NodeGroupVipAssignment.find_by_node_group_id_and_vip_id(parent.id, vip.id)
        if ngna.virtual_assignment?
          logger.debug "Removing NGNA from vip #{vip.name} to parent #{parent.name}"
  ...
  def remove_parent_virtuals(ng=self.node_group,nd=self.vip,excludes=[],depth=0)
    depth += 1
    # ask each parent group if they have a DIFF child group with that vip, either exclude that parent or add to delete list
    to_delete = []
    ...
    to_delete.uniq.each do |del_ng|
      ngna = NodeGroupVipAssignment.find_by_node_group_id_and_vip_id(del_ng.id, nd.id)
      unless excludes.include?(del_ng)
        if (defined?(ngna)) && (!ngna.nil?)
          if !ngna.virtual_assignment?
            ...
            NodeGroupVipAssignment.delete(ngna.id)
          end
        end
      else
      end
